	...
	for idx := range rules {
		WalkRules(rules[idx], func(rule *Rule) bool {
			...
			for i := range rule.Body {
				if op := rule.Body[i].OperatorTerm(); op != nil && internalPrintRef.Equal(op.Value) {
					skip = true
					break
				}
			}
...
func (i *baseDocEqIndex) Lookup(resolver ValueResolver) (*IndexResult, error) {
	...
	if !tr.multiple {
		...
		// TODO(ae): place this somewhere else?
		var lastValue Value
...
type refindices struct {
	...
	frequency *util.TypedHashMap[Ref, int]
	sorted    []Ref
}
...
func newrefindices(isVirtual func(Ref) bool) *refindices {
	return &refindices{
		isVirtual: isVirtual,
		rules:     map[*Rule][]*refindex{},
		frequency: util.NewTypedHashMap[Ref, int](refEqual),
	}
}
...
type trieNode struct {
	...
	any       *trieNode
	undefined *trieNode
	scalars   *util.TypedHashMap[Value, *trieNode]
	array     *trieNode
	rules     []*ruleNode
	value     *Term
	multiple  bool
...
func newTrieNodeImpl() *trieNode {
	return &trieNode{
		scalars: util.NewTypedHashMap[Value, *trieNode](valueEq),
	}
}
