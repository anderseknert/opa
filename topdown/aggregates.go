...
func builtinCount(_ BuiltinContext, operands []*ast.Term, iter func(*ast.Term) error) error {
	...
	case *ast.Array:
		return iter(ast.CachedIntNumberTerm(a.Len()))
	case ast.Object:
		return iter(ast.CachedIntNumberTerm(a.Len()))
	case ast.Set:
		return iter(ast.CachedIntNumberTerm(a.Len()))
	case ast.String:
		return iter(ast.CachedIntNumberTerm(len([]rune(a))))
	}
...
func builtinAll(_ BuiltinContext, operands []*ast.Term, iter func(*ast.Term) error) error {
	...
	case ast.Set:
		res := true
		match := ast.CachedBooleanTerm(true)
		...
		return iter(ast.CachedBooleanTerm(res))
	case *ast.Array:
		res := true
		match := ast.CachedBooleanTerm(true)
		val.Until(func(term *ast.Term) bool {
...
func builtinAny(_ BuiltinContext, operands []*ast.Term, iter func(*ast.Term) error) error {
	switch val := operands[0].Value.(type) {
	case ast.Set:
		res := val.Len() > 0 && val.Contains(ast.CachedBooleanTerm(true))
		return iter(ast.CachedBooleanTerm(res))
	case *ast.Array:
		res := false
		match := ast.CachedBooleanTerm(true)
...
func builtinMember(_ BuiltinContext, operands []*ast.Term, iter func(*ast.Term) error) error {
	...
	case ast.Set:
		return iter(ast.CachedBooleanTerm(c.Contains(containee)))
	case *ast.Array:
		for i := 0; i < c.Len(); i++ {
	...
	case ast.Object:
		return iter(ast.CachedBooleanTerm(c.Until(func(_, v *ast.Term) bool {
...
func builtinMemberWithKey(_ BuiltinContext, operands []*ast.Term, iter func(*ast.Term) error) error {
	...
	case interface{ Get(*ast.Term) *ast.Term }:
		...
		return iter(ast.CachedBooleanTerm(ret))
	}
	return iter(ast.CachedBooleanTerm(false))
}
